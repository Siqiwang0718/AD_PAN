...
library(GSEABase)
library(GSVA)
workingDir = "C:/Users/sqyql/Desktop/AD_PDF/Micro-array/ssGSEA/TUBA1B"
setwd(workingDir)
sgene="TUBA1B"       #输入进行单基因GSEA的基因名称
rt=as.matrix(rt)
rownames(rt)=rt[,1]
exp=rt[,2:ncol(rt)]
...
write.table(rt2,file="norexp_AD_TUBA1B.txt",sep="\t",quote=F,col.names = F)
...
write.table(DIFFOUT,file=paste0("1.","DIFF_all_TUBA1B_AD.xls"),sep="\t",quote=F,col.names=F)
...
pdf(file=paste0("2.", "DIFF_heatmap_TUBA1B_AD.pdf"),height=7,width=6)
...
#导出保存
write.csv(kegg, "4_all_GSEA_TUBA1B_AD.csv")


#5、可视化排序后分别取GSEA结果的前10个
num=10
pdf(paste0("4.","GSEA_TUBA1B_AD.pdf"),width = 9,height = 7)
